...
| P2-E | In progress — migration parser fix awaiting CI | Additive APP migration `0012`; explicit human RACI and separate policy configuration; fail-closed approval readiness; deterministic schema/catalog/dependency/alias/relationship checks after EffectiveScope; SoD/quorum and idempotency; atomic D1 normal and emergency publication; immutable audit/history; runtime suspension/resume and post-review; capability-gated Semantic Registry approval UI. No P2-F runtime semantic injection and no production semantic data were created. | Initial remote migration was rejected with incomplete input before any P2-E tables existed. Root cause was Wrangler splitter interaction with nested trigger `CASE`; equivalent `IIF` trigger expressions now pass local `d1 migrations apply` (19 statements). The corrected commit requires CI, then remote migration/deploy/smoke will be retried. |
