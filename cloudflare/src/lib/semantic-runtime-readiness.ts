...
interface DependencyEdge { revision_id: string; referenced_revision_id: string; }

const code = (status: ReadinessStatus, value: string) => ({ status, code: value });

function hasDependencyCycle(edges: DependencyEdge[]): boolean {
  const graph = new Map<string, string[]>();
  for (const edge of edges) graph.set(edge.revision_id, [...(graph.get(edge.revision_id) ?? []), edge.referenced_revision_id]);
  ...
  const visit = (revision: string): boolean => {
    ...
    const cycle = (graph.get(revision) ?? []).some(visit);
    visiting.delete(revision); visited.add(revision);
    return cycle;
  };
...
export async function semanticRuntimeActivationReadiness(env: Env): Promise<SemanticRuntimeReadiness> {
  ...
  const [policy, registry, snapshot, eligiblePublished, approved, invalidSources, invalidDependencies, dependencyEdges, appTables, dataTables] = await Promise.all([
    policyState(env),
    ...
    env.QUERYMIND_APP.prepare("SELECT s.revision_id, s.referenced_revision_id FROM semantic_sources s JOIN semantic_assets a ON a.current_approved_revision_id = s.revision_id JOIN semantic_revisions r ON r.revision_id = s.revision_id AND r.asset_id = a.asset_id JOIN semantic_publications p ON p.asset_id = a.asset_id AND p.revision_id = r.revision_id WHERE s.source_kind = 'SEMANTIC_DEPENDENCY' AND a.asset_status = 'ACTIVE' AND r.revision_status = 'APPROVED' AND p.runtime_eligibility = 'ELIGIBLE' ORDER BY s.revision_id, s.ordinal_position LIMIT 257").all<DependencyEdge>(),
  ...
  const dependencyCycle = (dependencyEdges.results?.length ?? 0) > 256 || hasDependencyCycle(dependencyEdges.results ?? []);
  const structuralIntegrity = (invalidSources?.total ?? 0) === 0 && (invalidDependencies?.total ?? 0) === 0 && !dependencyCycle;
  ...
  return {
    ...
    checks: {
      ...
      dependencies: code(structuralIntegrity ? "PASS" : "BLOCKED", structuralIntegrity ? "DEPENDENCIES_RESOLVE" : dependencyCycle ? "SEMANTIC_DEPENDENCY_CYCLE" : "DEPENDENCY_OR_SOURCE_INVALID"),
