...
type UserRow = { id: string; email: string; display_name: string; role_name: string; data_scope_key?: string | null; is_active: number; created_at: string; last_seen_at: string | null };
...
function publicUser(row: UserRow): Record<string, unknown> { return { id: row.id, email: row.email, displayName: row.display_name, roleName: row.role_name, dataScopeKey: row.data_scope_key || `role:${row.role_name}`, isActive: row.is_active === 1, createdAt: row.created_at, lastSeenAt: row.last_seen_at }; }
...
export async function exportCsv(request: Request, env: Env): Promise<Response> {
  ...
  if (!scope.canExport) throw new HttpError(403, "EXPORT_NOT_ALLOWED", "Your data scope does not allow export.");
...
export async function listUsers(request: Request, env: Env): Promise<Response> { await owner(request, env); const result = await env.QUERYMIND_APP.prepare("SELECT id, email, display_name, role_name, data_scope_key, is_active, created_at, last_seen_at FROM users ORDER BY created_at ASC").all<UserRow>(); return json({ users: (result.results ?? []).map(publicUser) }); }

export async function updateUser(request: Request, env: Env, id: string): Promise<Response> { const actor = await owner(request, env); const body = objectBody(await readJson(request)); const target = await env.QUERYMIND_APP.prepare("SELECT id, email, display_name, role_name, data_scope_key, is_active, created_at, last_seen_at FROM users WHERE id = ?").bind(id).first<UserRow>(); if (!target) throw new HttpError(404, "USER_NOT_FOUND", "User was not found."); const nextRole = body.roleName === undefined ? target.role_name : roleName(body.roleName); const nextActive = booleanValue(body.isActive, "isActive") ?? Boolean(target.is_active); const requestedScope = body.dataScopeKey === undefined ? target.data_scope_key ?? "" : optionalText(body.dataScopeKey, "dataScopeKey", 120) ?? ""; if (requestedScope && !/^[A-Za-z0-9:_-]{1,120}$/u.test(requestedScope)) throw new HttpError(400, "INVALID_REQUEST", "dataScopeKey contains unsupported characters."); if (target.id === actor.id && (!nextActive || nextRole !== "owner")) throw new HttpError(400, "OWNER_PROTECTION", "You cannot remove your own Owner access."); const removingActiveOwner = target.role_name === "owner" && target.is_active === 1 && (nextRole !== "owner" || !nextActive); const now = new Date().toISOString(); const result = await env.QUERYMIND_APP.prepare(`UPDATE users SET role_name = ?, data_scope_key = ?, is_active = ?, updated_at = ? WHERE id = ?${removingActiveOwner ? " AND EXISTS (SELECT 1 FROM users AS other WHERE other.id <> users.id AND other.role_name = 'owner' AND other.is_active = 1)" : ""}`).bind(nextRole, requestedScope, Number(nextActive), now, id).run(); if (!result.meta.changes) throw new HttpError(400, "OWNER_PROTECTION", "At least one active Owner is required."); await audit(env, { actorId: actor.id, eventType: "user.updated", resourceType: "user", resourceId: id, metadata: { roleName: nextRole, dataScopeKey: requestedScope || `role:${nextRole}`, isActive: nextActive } }); return json({ user: publicUser({ ...target, role_name: nextRole, data_scope_key: requestedScope, is_active: Number(nextActive) }) }); }
