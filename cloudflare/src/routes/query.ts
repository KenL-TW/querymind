...
async function recordQueryRun(
  ...
  const statements = [
    env.QUERYMIND_APP.prepare(
      "INSERT INTO query_runs (id, session_id, user_id, prompt, generated_sql, validated_sql, row_count, duration_ms, outcome, error_code, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ).bind(runId, input.sessionId, input.userId, input.prompt ?? input.generatedSql, input.generatedSql, input.generatedSql, input.rowCount, input.durationMs, input.outcome, input.errorCode, now),
    env.QUERYMIND_APP.prepare(
  ...
  if (input.sessionId && input.outcome === "success" && input.result) {
    ...
    statements.push(
      ...
      env.QUERYMIND_APP.prepare("INSERT INTO chat_messages (id, session_id, role, content, metadata_json, created_at) VALUES (?, ?, 'assistant', ?, ?, ?)").bind(
        ...
        JSON.stringify({ sql: input.generatedSql, rows: preview, rowCount: input.rowCount, truncated: input.result.rows.length > preview.length, maskedColumns: input.result.maskedColumns, durationMs: input.durationMs, source: "direct-query" }),
        now,
      ),
...
export async function executeQuery(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const payload = { rows: masked.rows, rowCount: rows.length, rowCap: validated.rowCap, maskedColumns: masked.maskedColumns, durationMs };
    assertApiResultBudget(payload);
    await recordQueryRun(env, { sessionId, userId: user.id, prompt, generatedSql: validated.originalSql, rowCount: rows.length, durationMs, outcome: "success", errorCode: null, result: masked });
    return Response.json(payload, { headers: { "cache-control": "no-store" } });
  } catch (error) {
    const durationMs = Date.now() - startedAt;
    const code = error instanceof HttpError ? error.code : "QUERY_EXECUTION_FAILED";
    await recordQueryRun(env, { sessionId, userId: user.id, prompt, generatedSql: validated.originalSql, rowCount: 0, durationMs, outcome: "error", errorCode: code });
    throw error;
  }
}
