...
const manifestPath = path.join(root, "..", "docs", "releases", "manifests", "p2-d-production.json");
...
export function validateReleaseManifest(value) {
  ...
  if (value.phase !== "P2-D") fail("phase must be P2-D");
...
export function loadReleaseManifest(file = manifestPath) {
  return validateReleaseManifest(JSON.parse(readFileSync(file, "utf8")));
}

if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const value = loadReleaseManifest(process.argv[2] ? path.resolve(process.argv[2]) : manifestPath);
